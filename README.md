...
📷 **이미지 : 13-1.png (MEMORY_LIMIT=512 적용 화면)**
![MEMORY_LIMIT=512 적용 화면](./img/13-1.png)
...
📷 **이미지 : 7-1.png (After - CPU Limit 50%, OK)**
![CPU_MAX_OCCUPY=50 설정 후 OK 상태](./img/7-1.png)
...
📷 **이미지 : 7-1.png (After - STABLE 상태)**
![MULTI_THREAD_ENABLE=False, SYSTEM STATUS : STABLE](./img/7-1.png)
...
예를 들어 MEMORY_LIMIT이나 CPU_MAX_OCCUPY를 변경했을 때 단순히 "더 오래 실행되었다"라고 서술하는 것이 아니라, **프로세스 생존 시간, 최대 메모리 사용량, CPU 사용률 변화, 종료 시점(Timestamp), 종료 원인 로그** 등을 표와 그래프로 함께 정리하여 변화량을 수치로 제시하고 싶다. 또한 Deadlock 분석에서는 PID 존재 여부, WAITING/BLOCKED 로그 발생 시점, CPU와 메모리 변화 추이를 함께 기록하여 실제로 프로세스는 살아있지만 작업이 멈춘 상태였다는 점을 더욱 명확하게 입증할 것이다.

이번 미션을 진행하면서 단순히 로그를 확인하는 것보다 **여러 관제 데이터와 시스템 정보를 함께 수집하여 상호 비교하는 과정이 장애 원인을 더욱 객관적으로 분석하는 데 중요하다는 점**을 배울 수 있었다. 다음에는 이러한 데이터를 지속적으로 수집하고 시각화하여, 추측이 아닌 수치와 근거를 기반으로 한 더욱 신뢰성 높은 장애 분석 리포트를 작성하고자 한다.
